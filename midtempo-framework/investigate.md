...
- [Step 2: Clarify Concern](#step-2-clarify-concern)
  ...
  - [External Research Scope](#215-external-research-scope)
...
- [Step 4: Analyse & Synthesise](#step-4-analyse--synthesise)
  ...
  - [Mechanism](#42-mechanism)
  - [Current State](#43-current-state)
  - [What Could Happen](#44-what-could-happen)
  - [Glossary](#45-glossary)
  - [Impact Assessment](#46-impact-assessment)
  - [Ownership Map](#47-ownership-map)
  - [Present Analysis](#48-present-analysis)
...
INVENTORY all design documents (feeds §5 ownership):
  SCAN planning/**/*-design.md — active docs
  SCAN planning/archive/**/*-design.md — archived docs (skip if directory absent)
  FOR EACH doc found:
    RECORD: path | active/archived | date from `**Date:**` header | substantive/adjacent
    Substantive: specifies multiple functionality points
    Adjacent: specifies only one neighbouring point
...
IF opening prompt describes observations with enough detail to frame an investigation question:
  ...
  IF human confirms → Continue to §2.1.5 External Research Scope
...
### 2.1.5 External Research Scope

After the framing reflection is confirmed, ask whether the investigation should include external research.

**Before asking:** Based on Step 1 context, identify any external sources that would meaningfully inform the investigation (vendor docs, RFCs, domain-specific references, third-party library docs). Populate option B with these specific areas. If nothing obvious surfaces, option B may state "no specific areas — general background research".

**Present options to the human:**

```
External research for this investigation:

  A) No external research — codebase, docs, and trace evidence are enough
  B) Yes, focus on: [agent-suggested areas — populated from Step 1 context]
  C) Yes, focus on: [your areas — please specify]
```
...
```
IF human selects A
  → SET research_opt_in = false
  → SET research_hints = none

IF human selects B
  → SET research_opt_in = true
  → SET research_hints = [agent-suggested areas presented in option B]

IF human selects C
  → SET research_opt_in = true
  → ASK: "What sources or topics should the agent focus on?"
  → WAIT for answer
  → SET research_hints = [human-specified topics]
```

After flags set → Continue to §2.2 Define Scope
...
- External research: [none / research_hints from §2.1.5]
...
**External Research:** [On — focus: research_hints / Off]
...
**External research runs when `research_opt_in` == true (set in §2.1.5).**

```
TRIGGER:
  IF research_opt_in == true (set in §2.1.5)
    → Use research_hints (set in §2.1.5) as the focus areas for CHECK-RESOURCES and FETCH
    → Proceed with CHECK-RESOURCES, FETCH, and DOCUMENT below
  IF research_opt_in == false
    → SKIP §3.4 — proceed to §3.5 Present Evidence
...
**Connect evidence to find patterns, root causes, and a complete understanding of the situation.**
...
IF `research_opt_in` == false → SKIP §4.5

For each unfamiliar term encountered during external research:
...
Map ownership evidence for each behaviour-changing candidate before §5 classification.

```
FOR EACH behaviour-changing candidate:
  LOOK UP from §1.1 inventory: docs that specify this behaviour
  OWNED: any substantive doc (active or archived) specifies it — most recent wins
  UNOWNED: no substantive doc specifies it

DOCUMENT (one entry per candidate):

### Ownership: [Behaviour description]

**Owning doc:** [path (active/archived, YYYY-MM-DD)] | none
**Owned?** OWNED | UNOWNED
```

### 4.8 Present Analysis
...
Glossary: [N terms defined, or "N/A — no external research"]
...
Ownership map:
- [Behaviour]: OWNED — [path (active/archived, YYYY-MM-DD)]
- [Behaviour]: UNOWNED

Ready to propose recommendations?
```
...
- §4 Analysis — Pattern Recognition, Mechanism, Current State, Ownership Map, What Could Happen, Glossary (if applicable), Impact Assessment, root causes and anomalies from the Step 4 output above
...
### 5.0 Cost & Ownership Principle

**Build is expensive.** It manufactures a design doc and plan through a fixed pipeline (~2–3 hr). When a design doc already exists for the behaviour, that pipeline is waste — Refine runs one contained TDD flow off the existing doc. Ownership (does a design doc specify this behaviour?) only ever forks Refine-vs-Build. Bug and Refactor bypass ownership entirely.

**Investigation is a last resort.** Classify as Investigation only when no other classification fits. Never spawn another investigation from an investigation.

Apply this procedure to every recommendation before running §5.1 gates:

0. Schema change (new tables or columns)?
   → Isolate as a standalone Refactor (dependency-linked). Never bundle with other work.
1. Broken behaviour (root cause found)?
   → Bug. Ownership irrelevant.
2. Behaviour-preserving cleanup?
   → Refactor. Ownership irrelevant. ≤15 files.
3. Behaviour-changing work?
   Determine ownership first:
   A behaviour is OWNED if a design doc — active or archived, substantive (multiple
   functionality points, not merely adjacent) — specifies it. Most recent wins.
   Archived means surface not actively worked on, not deleted.

   OWNED → Refine. Too big for Refine gates → split into more Refines. Never re-Build.
   UNOWNED → Build. Oversize → surface-separated Builds, or core-Build + follow-on Refines.
              No clean split? → Investigation (last resort — never spawns another investigation).
...
IF work is a foundational task (schema migration, scaffolding) that enables a dependent recommendation
  → VALID: May stand alone — declare the dependency link in both recommendation files
...
IF behaviour is UNOWNED per §5.0 step 3
  → INVALID: STOP — Unowned behaviour requires Build skill.

IF work affects 4+ files
  → INVALID: Split into more Refines (each ≤3 files, ≤3 criteria). Never re-Build owned behaviour.
...
IF work is an isolated schema migration (standalone Refactor per §5.0 step 0)
  → VALID: Use refactor skill
  TYPE: migration — declare dependency link in both recommendation files
...
IF work contains schema changes (new tables or columns) bundled with logic changes
  → EXTRACT the schema change as a standalone Refactor (§5.0 step 0, §5.1.2)
  → Re-classify remaining logic work from the top of §5.1

IF work affects 16+ files
  → Split into surface-separated Builds (one Build per surface or domain boundary)
  → Classify as Investigation only if no clean surface split exists
...
Investigation is the last resort. Classify here only when no other classification fits and surface-separated Builds (§5.1.3) cannot resolve the scope. Never spawn another investigation from an investigation.

**Uncertainty:** The evidence gathered in §3 is insufficient to write a specific recommendation. Use only when the investigation cannot determine even the likely classification.

**Decomposition:** Work exceeds Build size limits and no clean surface boundary isolates actionable chunks. Build's decisions and design phases resolve decomposition — this trigger applies only when the surfaces are genuinely unclear.
...
1. Broken behaviour → Bug (§5.1 criteria; size irrelevant)
2. Behaviour-preserving restructure → Refactor (§5.1 criteria; size ceiling ≤15 files)
3. Behaviour-changing, ANY signal exceeding Build column → Investigation
4. Behaviour-changing, ANY signal exceeding Refine column → Build (not Refine)
5. Behaviour-changing, ALL signals within Refine column AND §5.1.1 criteria pass → Refine
...
```
Schema change: [yes → isolate as standalone Refactor (§5.0 step 0) — stop checklist here | no]
Files affected: [N]
Acceptance criteria: [N]
Estimated tests: [N]
Modules/domains: [N]
New patterns: [N]
Owning design doc: [path (active/archived, date) | none — archived counts as owned]
...
| # | Title | Classification | Owned by | Est. Tests | Modules | Files | Risk |
|---|-------|---------------|----------|------------|---------|-------|------|
| 1 | [Title] | [Classification] | [active / archived (date) / none] | [N] | [N] | [N] | [Low/Med/High] |
| 2 | [Title] | [Classification] | [active / archived (date) / none] | [N] | [N] | [N] | [Low/Med/High] |
...
1. [Title] (Classification: [Classification])
   ...
   Owned by: [design doc path (active/archived, YYYY-MM-DD), or "none — unowned"]
   Surface: [surface/domain this Build covers — for surface-separated Builds; "single surface" otherwise]
   Schema: [migration — linked to rec #N; or "none"]
...
| Priority | Title   | Classification | Owned by | File                     |
| -------- | ------- | -------------- | -------- | ------------------------ |
| 1        | [Title] | [Classification] | [active / archived (date) / none] | `[name]-rec-1-[slug].md` |
| 2        | [Title] | [Classification] | [active / archived (date) / none] | `[name]-rec-2-[slug].md` |
...
| Priority | Title | Classification | Owned by | Est. Tests | Modules | Risk |
|----------|-------|----------------|----------|------------|---------|------|
| 1 | [Title] | [Classification] | [active / archived (date) / none] | [N] | [N] | [Low/Med/High] |
| 2 | [Title] | [Classification] | [active / archived (date) / none] | [N] | [N] | [Low/Med/High] |
