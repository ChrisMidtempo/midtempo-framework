...
CONFIRM the located design doc owns the feature being refined:
  READ its §2.2 Explicitly Out of Scope.
  IF the refinement changes behaviour in a layer §2.2 excludes
    → STOP: "This refinement changes [layer], which this design doc marks out of scope.
       Which design doc owns it?"
    → Confirm the correct doc before recording anything in Step 8.
...
**Reconcile the authoritative spec (§1–8) before recording the refinement.**

The design doc's numbered sections are the current-state specification. A refinement that changes behaviour makes them stale.

1. IDENTIFY every acceptance criterion or §3.x section the refinement changed
2. EDIT each in place so it describes the refined behaviour
3. IF a prior decision is now invalid → delete the stale text, never annotate it

IF any affected AC / §3.x still describes the old behaviour
  → INVALID: STOP — reconcile the spec before recording the refinement

**Record the change.** Add one row to the design doc's Change History table — nothing more:

| Date | Type | Summary | Commit |
|------|------|---------|--------|
| DD/MM/YYYY | refine | [what was tweaked — one line] | [commit ref] |

The refinement context, rationale, and learnings live in the commit body, not the doc.
...
**INVALID:** Append a free-form refinement narrative to the design doc.

**VALID:** Reconcile the affected spec sections in place, then record one Change History row — the rationale lives in the commit.
