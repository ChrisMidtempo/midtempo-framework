...
1. **Copy files**
    ...
    Copy the generated docs into your repository's root in a folder called `/midtempo-framework/`

2. **Move CLAUDE.md file**

    Move the CLAUDE.md file from the `/midtempo-framework/` folder into the repository root.

3. **Run Setup** (9 stages)

Start a new conversation with a coding LLM - Claude Sonnet provides consistent results - using the following prompt:
...
There are nine setup stages:
  ...
  - [1] Command alignment validation

  - [2-4] Mandatory Instructions (purpose, architecture, error-handling)

  - [5-8] Conditional Documents (database, frontend, style guide, wiring a new page)
...
```
Run /midtempo-framework/investigate.md 
Read the /midtempo-framework/instructions/*.md documents 
Identify areas of concern that should be addressed and discuss
```